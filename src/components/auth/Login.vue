<template>
  <v-app id="inspire">
    <v-img
      ...
      :src="require('@/assets/chimarrao.jpg')"
    ...
    async submit() {
      if (this.isLogin) {
        try {
          ...
          var id_avatar = response.data.user.id_avatar;
          var id_address = response.data.user.id_address;

          if (id_avatar) {
            const { _id: idAvatar, url } = response.data.user.id_avatar;
